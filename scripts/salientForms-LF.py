...
# Load the forms.csv file
forms_df = pd.read_csv('../cldf/forms.csv')

# Load the allSalienceScores-LF.tsv file
salience_scores_df = pd.read_csv('../analyses/segments/allSalienceScores-LF.tsv', sep='\t')
...
for index, row in forms_df.iterrows():
    ...
    parameter_id = row['Parameter_ID'].split('_')[1] # get rid of ID number
...
highest_salience_df.to_csv('../analyses/segments/highestSalienceForms.tsv', index=False, sep="\t")
