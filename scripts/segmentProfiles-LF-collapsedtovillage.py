...
for index, row in forms.iterrows():
	...
	# collapse doculect to village varieties
	doculect = row["Language_ID"]
	docIDpattern = r'[A-Z]{3}([a-zA-Z]+)\d+' # r' is for a "raw" string
	village = re.search(docIDpattern, doculect).group(1)
	doculectToVillage[doculect] = village
...
for index, row in forms.iterrows():
	...
	doculect = row["Language_ID"]
	doculect = doculectToVillage[doculect]
...
normedSegCountsDF.T.to_csv("../analyses/segments/normedSegmentCounts-LFbyVillage.tsv", sep="\t")
...
segDistsDF.to_csv("../analyses/segments/segmentProfiles-LFbyVillage.tsv", sep="\t", float_format='%.9f' ) # had some weird floating point issue where 1 == .99999
...
doculectPairsDF.to_csv("../analyses/segments/segmentProfilesByPair-LFbyVillage.tsv", sep="\t", float_format='%.9f' )
...
segstdevsDF.to_csv("../analyses/segments/segmentSTDevs-LFbyVillage.tsv", sep="\t", index=False)
...
for doculect, segzscores in segszscores.iterrows():
	...
	for seg, zscore in segzscores.items():
		...
		#if abs(zscore) > 2:
		if zscore > 2: # just do positive for now
...
print(compiledzscores, file=open("../analyses/segments/segmentZScores-LFbyVillage.tsv", "w"))
print(highestsaliencescores, file=open("../analyses/segments/highestSalienceScores-LFbyVillage.tsv", "w"))
print(allsaliencescores, file=open("../analyses/segments/allSalienceScores-LFbyVillage.tsv", "w"))
