"""
Run cognate analyses of Grollemund wordlists and produce associated outputs
"""

from lingpy import *
from lexibank_grollemund import Dataset
...
# See this for why I had such install difficulties:
# https://github.com/pypa/setuptools/issues/3548

#HYPHENS REMOVED FOR THIS EXERCISE (originally, but now using CLDF data with boundaries)
...
def run(args):
	...
	# This is an inefficient set of processes since it repeats a lot of calculations for each entry
	# This is due to laziness on my part -JG
	doculectCoverage = { }
	for idx in origLex:
		doculect = origLex[idx, "doculect"]
		try:
			doculectCount = doculectCoverage[doculect]
			doculectCoverage[doculect] = doculectCount + 1
		except: doculectCoverage[doculect] = 1
		
	for doculect in doculectCoverage.keys():
		coverage = doculectCoverage[doculect]
		if coverage >= docCoverageThreshold:
			numDoculects = numDoculects + 1
	...
	for idx in origLex:
		...
		if filledness >= filledFraction:
			if doculectCoverage[doculect] >= docCoverageThreshold:
				try: selectedConcepts[concept] = selectedConcepts[concept] + 1
	...
	outputFile = open(analysesFolder + "/" + analysesSubfolder + "/" + filePrefix + "-DoculectCoverage" + ".tsv", "w")
	for doculectCovPair in sorted(doculectCoverage.items(), key = lambda kv: (-kv[1], kv[0])): # sort by numeric value, the alphabetically by key
		doculect, coverage = doculectCovPair
		output = doculect + "\t" + str(coverage)
		outputFile.write(output + "\n")
	outputFile.close()
	...
	# Skipping LexStat for now, too slow
	try:
