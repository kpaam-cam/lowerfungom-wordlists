...
dists <-
  read.csv(  '../analyses/kplfSubset-SCA-0.45_threshold-heatmap.matrix.dst',
    sep = "\t"
  )
...
clusterLabels = list(
	...
	c("Mungbam","Other","Mufu-Mundabli"),
	c("Mungbam", "Kung","Mufu-Mundabli","Other"),
	c("Mungbam","Kung","Mufu-Mundabli","Other","Mashi"),
	c("Mungbam","Kung","Mufu-Mundabli","Mixed 1","Mixed2","Mashi" ),
	c("Mungbam","Kung","Mufu-Mundabli","Mixed 1","Mixed 2","Ajumbu","Mashi"),
	c("Mungba","Missong","Kung","Mufu-Mundabli","Mixed 1","Mixed 2","Ajumbu","Mashi"),
	c("Mungba","Missong","Kung","Mufu-Mundabli","Buu","Koshin","Fang","Ajumbu","Mashi"),
	c("Mixed Mungba 1","Mixed Mungba 2","Missong","Kung","Mufu-Mundabli","Buu","Koshin","Fang","Ajumbu","Mashi"),
	c("Mixed Mungba 1","Munken","Mixed Mungba 2","Missong","Kung","Mufu-Mundabli","Buu","Koshin","Fang","Ajumbu","Mashi"),
	c("Biya","Munken","Ngun","Abar","Missong","Kung","Mufu-Mundabli","Buu","Koshin","Fang","Ajumbu","Mashi"),
	c("Biya","Munken","Ngun","Abar","Missong","Kung","Mufu","Mundabli","Buu","Koshin","Fang","Ajumbu","Mashi")
	)

for( n in 2:13 ) {
	print(paste("Generating plot for", n, "clusters"))
	pdf(file=paste("../figures/clusters/", n ,".pdf", sep=""),
		width=10, height=8)
	print(autoplot(
		pam(dists, n),
		label = TRUE,
...
# Silhouette plot for five clusters (custom; PCA-matched colors and labels)

# Compute PAM
pam5 <- pam(dists, 5)

# Compute silhouette widths
sil5 <- cluster::silhouette(pam5$cluster, dists)

# Build plotting data
sil_df <- data.frame(
  doculect  = rownames(dists),
  cluster   = factor(pam5$cluster),
  sil_width = sil5[, 3],
  stringsAsFactors = FALSE
)

# Order for readability: cluster, then silhouette width
sil_df <- sil_df[order(sil_df$cluster, -sil_df$sil_width), ]
sil_df$doculect <- factor(sil_df$doculect, levels = sil_df$doculect)

# Match PCA / MDS color and label conventions
n <- 5
pal5 <- smooth_rainbow(n, range = c(0.25, 1))
lab5 <- clusterLabels[[n - 1]]

# Write to file
pdf(file = "../figures/SilhouetteFiveClusters.pdf",
    width = 8, height = 5)

ggplot(sil_df, aes(x = doculect, y = sil_width, fill = cluster)) +
  # silhouette bars
  geom_col(width = 0.7) +

  # doculect labels below the x-axis
geom_text(
  aes(y = 0, label = doculect, color = cluster),
  angle = 90,
  hjust = 1.1,   # moves labels up and down
  vjust = 0.25,   # center on column
  size = 2.5,
  show.legend = FALSE
	) +
  ...
  theme_classic() +
theme(
  axis.text.x  = element_blank(),  # removes the horizontal x-axis labels
  axis.ticks.x = element_blank(),  # optional, cleaner since we’re custom-labeling
  ...
  # allow labels to draw into margins
  ...
  # color scales
  scale_fill_manual("", values = pal5, labels = lab5) +
...
# I wanted a silhouette chart without a dotted "best" line. So, I adopted the function
# in the original library here
#############
