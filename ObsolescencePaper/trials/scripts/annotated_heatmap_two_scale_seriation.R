# Two-scale seriation variant of annotated_heatmap.R: hierarchical clustering
...
# the ordering.
# Run from this directory with: Rscript annotated_heatmap_two_scale_seriation.R

input <- "../Good-WestermannPaperSupplementalMaterials/analyses/kplfSubset-SCA-0.45_threshold-heatmap.matrix.dst"
png_out <- "annotated_subunit_heatmap_two_scale_seriation.png"
pdf_out <- "annotated_subunit_heatmap_two_scale_seriation.pdf"
heatmap_only_png <- "heatmap_only_two_scale_seriation.png"
heatmap_only_pdf <- "heatmap_only_two_scale_seriation.pdf"
...
ordered_fine <- unlist(lapply(macro_order, function(macro_id) {
  local_fine <- macro_indices[[macro_id]]
  local_order <- if (length(local_fine) > 1) {
    hclust(as.dist(fine_cost[local_fine, local_fine, drop = FALSE]),
           method = "average")$order
  } else 1L
  fine_paths[local_fine[local_order]]
}), use.names = FALSE)
...
crop_heatmap_only <- function() {
  ...
  if (nzchar(magick)) {
    trimmed_png <- file.path(tempdir(), "heatmap_only_two_scale_seriation_trimmed.png")
  ...
  if (nzchar(pdfcrop)) {
    cropped_pdf <- file.path(tempdir(), "heatmap_only_two_scale_seriation_cropped.pdf")
