...
# Run from this directory with: Rscript annotated_heatmap_full_seriation.R

input <- "../Good-WestermannPaperSupplementalMaterials/analyses/kplfSubset-SCA-0.45_threshold-heatmap.matrix.dst"
png_out <- "annotated_subunit_heatmap_full_seriation.png"
pdf_out <- "annotated_subunit_heatmap_full_seriation.pdf"
heatmap_only_png <- "heatmap_only_full_seriation.png"
heatmap_only_pdf <- "heatmap_only_full_seriation.pdf"
...
for (start in seq_len(n)) {
  candidate <- two_opt(nearest_neighbor_path(profile_cost, start), profile_cost)
  candidate_len <- path_length(candidate, profile_cost)
  if (candidate_len < best_len) {
    best_len <- candidate_len
    best_path <- candidate
  }
}
message("Full-leaf seriation path cost: ", round(best_len, 1))
...
crop_heatmap_only <- function() {
  ...
  if (nzchar(magick)) {
    trimmed_png <- file.path(tempdir(), "heatmap_only_full_seriation_trimmed.png")
  ...
  if (nzchar(pdfcrop)) {
    cropped_pdf <- file.path(tempdir(), "heatmap_only_full_seriation_cropped.pdf")
