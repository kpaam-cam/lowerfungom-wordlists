...
# the ordering.
# Run from this directory with: Rscript annotated_heatmap_two_scale_bridge_ajumbu_edge.R

input <- "../Good-WestermannPaperSupplementalMaterials/analyses/kplfSubset-SCA-0.45_threshold-heatmap.matrix.dst"
png_out <- "annotated_subunit_heatmap_two_scale_bridge_ajumbu_edge.png"
pdf_out <- "annotated_subunit_heatmap_two_scale_bridge_ajumbu_edge.pdf"
heatmap_only_png <- "heatmap_only_two_scale_bridge_ajumbu_edge.png"
heatmap_only_pdf <- "heatmap_only_two_scale_bridge_ajumbu_edge.pdf"
...
# Ajumbu participates freely in the matrix-based inference, but its inferred
# fine unit is moved to the beginning of the stored order. The display
# reverses that order, placing Ajumbu at the lower-right edge. Kung and Mashi
# remain wherever the two-scale ordering places them.
ajumbu_fine <- unique(fine_component[which(as.character(subunit) == "Ajumbu")])
if (length(ajumbu_fine) != 1) stop("Ajumbu did not form one inferred fine unit")
ajumbu_path <- fine_paths[[ajumbu_fine]]
ordered_fine <- c(ajumbu_path, ordered_fine[!ordered_fine %in% ajumbu_path])
message("Moved inferred Ajumbu fine unit to the display edge")
...
crop_heatmap_only <- function() {
  ...
  if (nzchar(magick)) {
    trimmed_png <- file.path(tempdir(), "heatmap_only_two_scale_bridge_ajumbu_edge_trimmed.png")
  ...
  if (nzchar(pdfcrop)) {
    cropped_pdf <- file.path(tempdir(), "heatmap_only_two_scale_bridge_ajumbu_edge_cropped.pdf")
