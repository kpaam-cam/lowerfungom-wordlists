...
# the ordering. Kung and Mashi are excluded before any structure is inferred.
# Run from this directory with: Rscript annotated_heatmap_two_scale_bridge_no_kung_mashi.R

input <- "../Good-WestermannPaperSupplementalMaterials/analyses/kplfSubset-SCA-0.45_threshold-heatmap.matrix.dst"
png_out <- "annotated_subunit_heatmap_two_scale_bridge_no_kung_mashi.png"
pdf_out <- "annotated_subunit_heatmap_two_scale_bridge_no_kung_mashi.pdf"
heatmap_only_png <- "heatmap_only_two_scale_bridge_no_kung_mashi.png"
heatmap_only_pdf <- "heatmap_only_two_scale_bridge_no_kung_mashi.pdf"
...
excluded <- as.character(subunit) %in% c("Kung", "Mashi")
...
crop_heatmap_only <- function() {
  ...
  if (nzchar(magick)) {
    trimmed_png <- file.path(tempdir(), "heatmap_only_two_scale_bridge_no_kung_mashi_trimmed.png")
  ...
  if (nzchar(pdfcrop)) {
    cropped_pdf <- file.path(tempdir(), "heatmap_only_two_scale_bridge_no_kung_mashi_cropped.pdf")
