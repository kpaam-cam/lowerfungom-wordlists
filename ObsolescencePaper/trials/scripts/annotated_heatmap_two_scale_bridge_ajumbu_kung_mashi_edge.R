...
# the ordering.
# Run from this directory with: Rscript annotated_heatmap_two_scale_bridge_ajumbu_kung_mashi_edge.R

input <- "../Good-WestermannPaperSupplementalMaterials/analyses/kplfSubset-SCA-0.45_threshold-heatmap.matrix.dst"
png_out <- "annotated_subunit_heatmap_two_scale_bridge_ajumbu_kung_mashi_edge.png"
pdf_out <- "annotated_subunit_heatmap_two_scale_bridge_ajumbu_kung_mashi_edge.pdf"
heatmap_only_png <- "heatmap_only_two_scale_bridge_ajumbu_kung_mashi_edge.png"
heatmap_only_pdf <- "heatmap_only_two_scale_bridge_ajumbu_kung_mashi_edge.pdf"
...
# Ajumbu, Kung, and Mashi form the lower-right edge group, but their order is
# inferred freely from the fine-unit distance matrix.
edge_names <- c("Ajumbu", "Kung", "Mashi")
edge_fine <- vapply(edge_names, function(name) {
  ids <- unique(fine_component[which(as.character(subunit) == name)])
  if (length(ids) != 1) stop(name, " did not form one inferred fine unit")
  ids
}, integer(1))
edge_cost <- fine_cost[edge_fine, edge_fine, drop = FALSE]
edge_path <- NULL
edge_length <- Inf
for (start in seq_along(edge_fine)) {
  candidate <- two_opt(nearest_neighbor_path(edge_cost, start), edge_cost)
  candidate_length <- path_length(candidate, edge_cost)
  if (candidate_length < edge_length) {
    edge_length <- candidate_length
    edge_path <- edge_fine[candidate]
  }
}
edge_leaves <- unlist(fine_paths[edge_path], use.names = FALSE)
ordered_fine <- c(edge_leaves, ordered_fine[!ordered_fine %in% edge_leaves])
message("Moved Ajumbu/Kung/Mashi edge group; internal order: ",
        paste(edge_names[match(edge_path, edge_fine)], collapse = " -> "))
...
crop_heatmap_only <- function() {
  ...
  if (nzchar(magick)) {
    trimmed_png <- file.path(tempdir(), "heatmap_only_two_scale_bridge_ajumbu_kung_mashi_edge_trimmed.png")
  ...
  if (nzchar(pdfcrop)) {
    cropped_pdf <- file.path(tempdir(), "heatmap_only_two_scale_bridge_ajumbu_kung_mashi_edge_cropped.pdf")
