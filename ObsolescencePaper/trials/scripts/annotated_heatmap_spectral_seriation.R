# Density-aware spectral-seriation variant of annotated_heatmap.R:
# is NOT used to determine order at all (kept only to draw a reference
# dendrogram panel). The ordering is derived from the normalized similarity
# graph's Fiedler vector, so dense regions can remain coherent without being
# specified as blocks. A profile-smoothness pass then makes local refinements.
# No variety or language labels affect the ordering.
# Run from this directory with: Rscript annotated_heatmap_spectral_seriation.R

input <- "../Good-WestermannPaperSupplementalMaterials/analyses/kplfSubset-SCA-0.45_threshold-heatmap.matrix.dst"
png_out <- "annotated_subunit_heatmap_spectral_seriation.png"
pdf_out <- "annotated_subunit_heatmap_spectral_seriation.pdf"
heatmap_only_png <- "heatmap_only_spectral_seriation.png"
heatmap_only_pdf <- "heatmap_only_spectral_seriation.pdf"
...
# Construct a normalized similarity graph. Degree normalization prevents a
# dense region from winning solely because it has many strong edges, while
# still favoring coherent high-similarity regions over scattered points.
similarity <- pmax(1 - dists, 0)
diag(similarity) <- 0
density <- rowSums(similarity)
normalized_similarity <- similarity / sqrt(outer(density, density))
laplacian <- diag(n) - normalized_similarity
spectral <- eigen(laplacian, symmetric = TRUE)
spectral_order <- order(spectral$vectors[, 2])

# Refine only by adjacent swaps, using the profile discontinuity cost. This
# preserves the continuous spectral ordering while removing obvious local
# reversals.
edge_scale <- median(profile_cost[upper.tri(profile_cost)])
edge_cost <- profile_cost / edge_scale
path_cost <- function(path) {
  sum(edge_cost[cbind(path[-length(path)], path[-1])])
}
ord <- spectral_order
improved <- TRUE
while (improved) {
  improved <- FALSE
  current_cost <- path_cost(ord)
...
message("Spectral graph density range: ", paste(round(range(density), 2), collapse = " - "))
message("Spectral/refined path cost: ", round(path_cost(ord), 2))
dists <- dists[ord, ord]
subunit <- subunit[ord]
labels <- labels0[ord]
...
crop_heatmap_only <- function() {
  ...
  if (nzchar(magick)) {
    trimmed_png <- file.path(tempdir(), "heatmap_only_spectral_seriation_trimmed.png")
  ...
  if (nzchar(pdfcrop)) {
    cropped_pdf <- file.path(tempdir(), "heatmap_only_spectral_seriation_cropped.pdf")
