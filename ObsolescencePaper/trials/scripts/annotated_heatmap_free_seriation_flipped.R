# Same as annotated_heatmap_free_seriation.R, but the final 180-degree
# rotation is skipped so MCANgun3 lands in the top-left corner instead of
# the bottom-right. This is a rigid point-reflection of the whole square:
# it does not re-run the optimizer or change any relative adjacency, it
# only mirrors which corner each end appears in.
# Run from this directory with: Rscript annotated_heatmap_free_seriation_flipped.R

input <- "../Good-WestermannPaperSupplementalMaterials/analyses/kplfSubset-SCA-0.45_threshold-heatmap.matrix.dst"
png_out <- "annotated_subunit_heatmap_free_seriation_flipped.png"
pdf_out <- "annotated_subunit_heatmap_free_seriation_flipped.pdf"
heatmap_only_png <- "heatmap_only_free_seriation_flipped.png"
heatmap_only_pdf <- "heatmap_only_free_seriation_flipped.pdf"
...
# Disabled in this alternate version: no manual fixing of the dark-blue
# (Missong/Abar/Ngun/Biya/Munken) run adjacency. find_run() is still needed
# below by the block-order optimizer's helper usage pattern, so it's defined
# but the reversal/swap steps that used it are commented out.
...
# The five blocks below (Koshin/Buu swap, Kung move, Mumfu-Mundabli move,
# Kung<->Mumfu-Mundabli swap, Mashi<->Mumfu-Mundabli swap) are disabled.
# Verified empirically (2026-07-17) that they have no effect on the final
# displayed order: each one only reorders runs that are already distinct,
# single-color blocks, and the exhaustive block-order optimizer below
# (search_block_orders) always finds the same global boundary-cost minimum
# regardless of the order those blocks arrive in. Left in place, commented
# out, as worked examples of the run/block-swap pattern for future reuse.
...
# Disabled here so MCANgun3 (position 1 in the pre-rotation order) stays in
# the top-left corner instead of being rotated to the bottom-right.
...
draw_main <- function(main_title = "Lower Fungom closeness heatmap (free seriation, axes reversed)",
