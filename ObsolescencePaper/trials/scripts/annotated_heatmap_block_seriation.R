...
# nearest-neighbor construction followed by 2-opt local search. Every
# language block remains contiguous, including Kung and Mashi, but no block
# has a constrained position in this first pass.
# Run from this directory with: Rscript annotated_heatmap_block_seriation.R

input <- "../Good-WestermannPaperSupplementalMaterials/analyses/kplfSubset-SCA-0.45_threshold-heatmap.matrix.dst"
png_out <- "annotated_subunit_heatmap_block_seriation.png"
pdf_out <- "annotated_subunit_heatmap_block_seriation.pdf"
heatmap_only_png <- "heatmap_only_block_seriation.png"
heatmap_only_pdf <- "heatmap_only_block_seriation.pdf"
...
block_result <- seriated_path(block_cost)
message("Language-block path cost: ", round(block_result$length, 1))

ord <- unlist(block_paths[block_result$path], use.names = FALSE)
dists <- dists[ord, ord]
subunit <- subunit[ord]
labels <- labels0[ord]
...
draw_main <- function(main_title = "Lower Fungom closeness heatmap (block seriation)",
...
crop_heatmap_only <- function() {
  ...
  if (nzchar(magick)) {
    trimmed_png <- file.path(tempdir(), "heatmap_only_block_seriation_trimmed.png")
  ...
  if (nzchar(pdfcrop)) {
    cropped_pdf <- file.path(tempdir(), "heatmap_only_block_seriation_cropped.pdf")
