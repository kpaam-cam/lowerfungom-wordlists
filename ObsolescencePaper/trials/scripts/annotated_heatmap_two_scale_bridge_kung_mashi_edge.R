...
# the ordering. Kung and Mashi are included in inference but placed together
# at the lower-right edge after ordering.
# Run from this directory with: Rscript annotated_heatmap_two_scale_bridge_kung_mashi_edge.R

input <- "../Good-WestermannPaperSupplementalMaterials/analyses/kplfSubset-SCA-0.45_threshold-heatmap.matrix.dst"
png_out <- "annotated_subunit_heatmap_two_scale_bridge_kung_mashi_edge.png"
pdf_out <- "annotated_subunit_heatmap_two_scale_bridge_kung_mashi_edge.pdf"
heatmap_only_png <- "heatmap_only_two_scale_bridge_kung_mashi_edge.png"
heatmap_only_pdf <- "heatmap_only_two_scale_bridge_kung_mashi_edge.pdf"
...
# Kung and Mashi are included in matrix-based inference, but their inferred
# fine units are moved to the beginning of the stored order. The display
# reverses that order, putting Mashi just inside Kung at the lower-right edge.
edge_fine <- vapply(c("Kung", "Mashi"), function(name) {
  ids <- unique(fine_component[which(as.character(subunit) == name)])
  if (length(ids) != 1) stop(name, " did not form one inferred fine unit")
  ids
}, integer(1))
edge_paths <- lapply(edge_fine, function(id) fine_paths[[id]])
edge_leaves <- unlist(edge_paths, use.names = FALSE)
ordered_fine <- c(edge_leaves, ordered_fine[!ordered_fine %in% edge_leaves])
message("Moved inferred Kung/Mashi fine units to the display edge")
...
crop_heatmap_only <- function() {
  ...
  if (nzchar(magick)) {
    trimmed_png <- file.path(tempdir(), "heatmap_only_two_scale_bridge_kung_mashi_edge_trimmed.png")
  ...
  if (nzchar(pdfcrop)) {
    cropped_pdf <- file.path(tempdir(), "heatmap_only_two_scale_bridge_kung_mashi_edge_cropped.pdf")
