# Bridge-aware full-leaf seriation variant of annotated_heatmap.R: hierarchical clustering
...
# two-sided bridge reward inferred from the data.
...
# No variety is named or constrained as a bridge.
# Run from this directory with: Rscript annotated_heatmap_bridge_seriation.R

input <- "../Good-WestermannPaperSupplementalMaterials/analyses/kplfSubset-SCA-0.45_threshold-heatmap.matrix.dst"
png_out <- "annotated_subunit_heatmap_bridge_seriation.png"
pdf_out <- "annotated_subunit_heatmap_bridge_seriation.pdf"
heatmap_only_png <- "heatmap_only_bridge_seriation.png"
heatmap_only_pdf <- "heatmap_only_bridge_seriation.pdf"
...
# Infer broad communities only for the bridge score. They do not impose
# contiguity or determine the order directly.
community_count <- 3L
...
bridge_score <- function(left, middle, right) {
  ...
  left_affinity <- mean(similarity[middle, community == left_community])
  right_affinity <- mean(similarity[middle, community == right_community])
  size_weight <- log1p(community_sizes[left_community]) *
    log1p(community_sizes[right_community])
  min(left_affinity, right_affinity) * size_weight
}
...
path_objective <- function(path) {
  ...
  edge_part - bridge_weight * bridge_part
}
...
message("Bridge-aware path objective: ", round(best_value, 2))
...
crop_heatmap_only <- function() {
  ...
  if (nzchar(magick)) {
    trimmed_png <- file.path(tempdir(), "heatmap_only_bridge_seriation_trimmed.png")
  ...
  if (nzchar(pdfcrop)) {
    cropped_pdf <- file.path(tempdir(), "heatmap_only_bridge_seriation_cropped.pdf")
