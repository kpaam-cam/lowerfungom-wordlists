...
# the ordering. Kung is excluded; Mashi is included in inference but placed
# at the lower-right edge after ordering.
# Run from this directory with: Rscript annotated_heatmap_two_scale_bridge_mashi_edge.R

input <- "../Good-WestermannPaperSupplementalMaterials/analyses/kplfSubset-SCA-0.45_threshold-heatmap.matrix.dst"
png_out <- "annotated_subunit_heatmap_two_scale_bridge_mashi_edge.png"
pdf_out <- "annotated_subunit_heatmap_two_scale_bridge_mashi_edge.pdf"
heatmap_only_png <- "heatmap_only_two_scale_bridge_mashi_edge.png"
heatmap_only_pdf <- "heatmap_only_two_scale_bridge_mashi_edge.pdf"
...
excluded <- as.character(subunit) %in% "Kung"
dists <- dists[!excluded, !excluded, drop = FALSE]
subunit <- droplevels(subunit[!excluded])
n <- nrow(dists)
...
# Mashi is included in the matrix-based inference, but its inferred fine unit
# is moved to the beginning of the stored order. The display reverses that
# order, putting Mashi at the lower-right edge.
mashi_fine <- unique(fine_component[which(as.character(subunit) == "Mashi")])
if (length(mashi_fine) != 1) stop("Mashi did not form one inferred fine unit")
mashi_path <- fine_paths[[mashi_fine]]
ordered_fine <- c(mashi_path, ordered_fine[!ordered_fine %in% mashi_path])
message("Moved inferred Mashi fine unit to the display edge")
...
crop_heatmap_only <- function() {
  ...
  if (nzchar(magick)) {
    trimmed_png <- file.path(tempdir(), "heatmap_only_two_scale_bridge_mashi_edge_trimmed.png")
  ...
  if (nzchar(pdfcrop)) {
    cropped_pdf <- file.path(tempdir(), "heatmap_only_two_scale_bridge_mashi_edge_cropped.pdf")
