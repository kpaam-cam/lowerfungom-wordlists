# Full-leaf seriation variant of annotated_heatmap.R: hierarchical clustering
# is NOT used to determine order at all (kept only to draw a reference
# dendrogram panel). Instead the unprotected leaves are placed directly
# by a minimum-cost Hamiltonian path (open TSP) over the same
...
# Mashi are constrained to terminal blocks: in the displayed heatmap Kung
# is the outer/lower-right block and Mashi is immediately inside it. The
# dark-blue varieties are also treated as one global-ordering block, while
# retaining their own internal seriation.
# Run from this directory with: Rscript annotated_heatmap_full_seriation_blue_block.R

input <- "../Good-WestermannPaperSupplementalMaterials/analyses/kplfSubset-SCA-0.45_threshold-heatmap.matrix.dst"
png_out <- "annotated_subunit_heatmap_full_seriation_blue_block.png"
pdf_out <- "annotated_subunit_heatmap_full_seriation_blue_block.pdf"
heatmap_only_png <- "heatmap_only_full_seriation_blue_block.png"
heatmap_only_pdf <- "heatmap_only_full_seriation_blue_block.pdf"
...
# Seriate Kung and Mashi internally, then place them at the beginning of the
# stored order. Since the display reverses the stored order, this puts Mashi
# just inside Kung at the lower-right corner.
protected_names <- c("Kung", "Mashi")
protected <- as.character(subunit) %in% protected_names

block_paths <- lapply(protected_names, function(block_name) {
  block_indices <- which(as.character(subunit) == block_name)
  block_result <- seriated_path(profile_cost[block_indices, block_indices, drop = FALSE])
  message(block_name, " internal seriation path cost: ", round(block_result$length, 1))
  block_indices[block_result$path]
})

# Treat all dark-blue varieties as one super-block for the global path. Its
# representative profile is the mean of the blue varieties' profiles. This
# makes the block's external placement depend on its aggregate profile rather
# than on whichever individual blue variety happens to be at an edge.
blue_names <- c("Biya", "Munken", "Ngun", "Abar", "Missong")
blue_indices <- which(as.character(subunit) %in% blue_names)
blue_result <- seriated_path(profile_cost[blue_indices, blue_indices, drop = FALSE])
blue_path <- blue_indices[blue_result$path]
blue_profile <- colMeans(dists[blue_indices, , drop = FALSE])
message("Blue-block internal seriation path cost: ", round(blue_result$length, 1))

individual_indices <- which(!protected & !as.character(subunit) %in% blue_names)
node_profiles <- rbind(blue_profile, dists[individual_indices, , drop = FALSE])
node_cost <- as.matrix(dist(node_profiles, method = "manhattan"))
node_result <- seriated_path(node_cost)
message("Blue-block/global-node path cost: ", round(node_result$length, 1))

free_path <- unlist(lapply(node_result$path, function(node) {
  if (node == 1) blue_path else individual_indices[node - 1]
}), use.names = FALSE)

ord <- c(unname(block_paths[[1]]), unname(block_paths[[2]]), free_path)
dists <- dists[ord, ord]
subunit <- subunit[ord]
labels <- labels0[ord]
...
draw_main <- function(main_title = "Lower Fungom closeness heatmap (blue block seriation)",
...
crop_heatmap_only <- function() {
  ...
  if (nzchar(magick)) {
    trimmed_png <- file.path(tempdir(), "heatmap_only_full_seriation_blue_block_trimmed.png")
  ...
  if (nzchar(pdfcrop)) {
    cropped_pdf <- file.path(tempdir(), "heatmap_only_full_seriation_blue_block_cropped.pdf")
