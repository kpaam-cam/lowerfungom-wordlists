...
# is the outer/lower-right block and Mashi is immediately inside it.
# Run from this directory with: Rscript annotated_heatmap_full_seriation_modified.R

input <- "../Good-WestermannPaperSupplementalMaterials/analyses/kplfSubset-SCA-0.45_threshold-heatmap.matrix.dst"
png_out <- "annotated_subunit_heatmap_full_seriation_modified.png"
pdf_out <- "annotated_subunit_heatmap_full_seriation_modified.pdf"
heatmap_only_png <- "heatmap_only_full_seriation_modified.png"
heatmap_only_pdf <- "heatmap_only_full_seriation_modified.pdf"
...
# Seriate all unprotected leaves without constraints. The protected blocks
# are each seriated internally, but remain contiguous and are placed at the
# beginning of the stored order. Since the display reverses the stored order,
# this puts Mashi just inside Kung at the lower-right corner.
protected_names <- c("Kung", "Mashi")
protected <- as.character(subunit) %in% protected_names
free_indices <- which(!protected)

free_result <- seriated_path(profile_cost[free_indices, free_indices, drop = FALSE])
free_path <- free_indices[free_result$path]
message("Unprotected-leaf seriation path cost: ", round(free_result$length, 1))
...
draw_main <- function(main_title = "Lower Fungom closeness heatmap (modified full seriation: Kung/Mashi blocks)",
...
crop_heatmap_only <- function() {
  ...
  if (nzchar(magick)) {
    trimmed_png <- file.path(tempdir(), "heatmap_only_full_seriation_modified_trimmed.png")
  ...
  if (nzchar(pdfcrop)) {
    cropped_pdf <- file.path(tempdir(), "heatmap_only_full_seriation_modified_cropped.pdf")
