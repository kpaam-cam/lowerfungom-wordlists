# Participation-based bridge-aware full-leaf seriation variant of
# annotated_heatmap.R:
...
# two-sided bridge reward and community cohesion inferred from the data.
...
# Run from this directory with: Rscript annotated_heatmap_participation_bridge_seriation.R

input <- "../Good-WestermannPaperSupplementalMaterials/analyses/kplfSubset-SCA-0.45_threshold-heatmap.matrix.dst"
png_out <- "annotated_subunit_heatmap_participation_bridge_seriation.png"
pdf_out <- "annotated_subunit_heatmap_participation_bridge_seriation.pdf"
heatmap_only_png <- "heatmap_only_participation_bridge_seriation.png"
heatmap_only_pdf <- "heatmap_only_participation_bridge_seriation.pdf"
...
# Infer broad communities only as a description of the matrix. They do not
# impose contiguity or determine the order directly. A variety's participation
# score is high when its similarity is distributed across communities rather
# than concentrated in one of them.
community_count <- 3L
...
community_affinity <- sapply(seq_len(community_count), function(k) {
  vapply(seq_len(n), function(i) {
    members <- which(community == k)
    members <- members[members != i]
    mean(similarity[i, members])
  }, numeric(1))
})
affinity_total <- rowSums(pmax(community_affinity, 0))
participation <- 1 - rowSums((community_affinity / affinity_total)^2)
...
bridge_score <- function(left, middle, right) {
  left_community <- community[left]
  right_community <- community[right]
  ...
  min(left_affinity, right_affinity) * size_weight * participation[middle]
}

cohesion_score <- function(left, right) {
  if (community[left] != community[right]) return(0)
  log1p(community_sizes[community[left]])
}
...
message("Participation score range: ", paste(round(range(participation), 3), collapse = " - "))
best_path <- NULL
best_value <- Inf
...
message("Participation bridge-aware path objective: ", round(best_value, 2))
...
crop_heatmap_only <- function() {
  ...
  if (nzchar(magick)) {
    trimmed_png <- file.path(tempdir(), "heatmap_only_participation_bridge_seriation_trimmed.png")
  ...
  if (nzchar(pdfcrop)) {
    cropped_pdf <- file.path(tempdir(), "heatmap_only_participation_bridge_seriation_cropped.pdf")
