...
# the ordering.
# Run from this directory with: Rscript annotated_heatmap_two_scale_bridge_seriation_flipped.R

input <- "../Good-WestermannPaperSupplementalMaterials/analyses/kplfSubset-SCA-0.45_threshold-heatmap.matrix.dst"
png_out <- "annotated_subunit_heatmap_two_scale_bridge_seriation_flipped.png"
pdf_out <- "annotated_subunit_heatmap_two_scale_bridge_seriation_flipped.pdf"
heatmap_only_png <- "heatmap_only_two_scale_bridge_seriation_flipped.png"
heatmap_only_pdf <- "heatmap_only_two_scale_bridge_seriation_flipped.pdf"
...
# Reverse both axes relative to the original output, placing the inferred
# Mungbam block at the upper-left of the displayed heatmap.
ord <- rev(ordered_fine)
dists <- dists[ord, ord]
subunit <- subunit[ord]
labels <- labels0[ord]
...
crop_heatmap_only <- function() {
  ...
  if (nzchar(magick)) {
    trimmed_png <- file.path(tempdir(), "heatmap_only_two_scale_bridge_seriation_flipped_trimmed.png")
  ...
  if (nzchar(pdfcrop)) {
    cropped_pdf <- file.path(tempdir(), "heatmap_only_two_scale_bridge_seriation_flipped_cropped.pdf")
