...
FlippedMdsLayout <- mdsLayout
#FlippedMdsLayout[,1] <- -FlippedMdsLayout[,1]
...
edgecolors = c("#CC772211", "#CC7722FF")

# Aspects of this remain magical to me
ggplot(ggnetwork(netGraph, layout=FlippedMdsLayout), # convert igraph to ggnetwork graph
       aes(x = x, y = y, xend = xend, yend = yend)) + # Set up graph base
  geom_edges(aes(color = weight, lwd=1.05^weight), show.legend=FALSE, curvature=.15 ) + 
  geom_nodes(color = "darkblue", size = 1) + scale_colour_gradientn(colours = edgecolors) +
  geom_nodetext_repel(aes(label = name), color = "darkblue", size = 2.5, segment.size = 0.1, max.overlaps=Inf) +
